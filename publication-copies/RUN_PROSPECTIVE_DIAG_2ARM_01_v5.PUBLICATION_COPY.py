...
PATCH2 = ROOT / "PROSPECTIVE-DIAG-2ARM-01_v0.2_PREOBS_PATCH.md"
FREEZE2 = ROOT / "PROSPECTIVE-DIAG-2ARM-01_v0.2_FREEZE_MANIFEST.json"
PATCH3 = ROOT / "PROSPECTIVE-DIAG-2ARM-01_v0.3_PREOBS_PATCH.md"
FREEZE3 = ROOT / "PROSPECTIVE-DIAG-2ARM-01_v0.3_FREEZE_MANIFEST.json"
PATCH4 = ROOT / "PROSPECTIVE-DIAG-2ARM-01_v0.4_PREOBS_PATCH.md"
FREEZE4 = ROOT / "PROSPECTIVE-DIAG-2ARM-01_v0.4_FREEZE_MANIFEST.json"

PREEXEC = ROOT / "RUNNER_PREEXEC_RECORD_v5.json"
RUN_DIR = ROOT / "EXECUTION_005"
...
EXPECTED = {
    ...
    PATCH2: "53414251ac97a5317f2193f88b02c37ea0c8e364d8a6de4b63f4488f16197aa0",
    FREEZE2: "e62a2d6cc7901c0592b1ef3e659ae2cf4b33f74c637c186cb55ca663b05b3394",
    PATCH3: "bde31549d8ab52fca15bfde6458eb7349bd1b8696fdb8d370ebfd63ce3171d53",
    FREEZE3: "9c4cc0ca0380788206793b67bd2394a68a8898fede9128396bd5800d8da921b1",
    PATCH4: "dbabf626671e45ba747d8344fd3921a81f2fdc06c97bfae64179a704ade4f134",
    FREEZE4: "ec0d065a7ffdde793b459680a56e412edf9d842edae1e2b064b5d6d8b9b5f5fb",
...
def extract_assistant_response(raw_stdout_bytes, user_prompt):
    try:
        stdout_text = raw_stdout_bytes.decode("utf-8", errors="strict")
    except UnicodeDecodeError as e:
        raise ValueError(
            f"RAW_STDOUT_NOT_STRICT_UTF8: {e}"
        ) from e
    ...
    prompt_pattern = re.compile(
        "".join(
            ...
            for character in user_prompt
        )
    )
    prompt_matches = list(prompt_pattern.finditer(stdout_text))

    if len(prompt_matches) != 1:
        raise ValueError(
            "FROZEN_USER_PROMPT_MATCH_COUNT_"
            f"{len(prompt_matches)}_EXPECTED_1"
        )

    candidate_start = prompt_matches[0].end()
    candidate_and_footer = stdout_text[candidate_start:]
    footer_prefix = "[ Prompt:"
    footer_start = (
        0 if candidate_and_footer.startswith(footer_prefix) else None
    )

    if footer_start is None:
        for index, character in enumerate(candidate_and_footer):
            if character in "\r\n" and candidate_and_footer.startswith(
                footer_prefix, index + 1
            ):
                footer_start = index + 1
                break

    if footer_start is None:
        raise ValueError("PROMPT_FOOTER_LINE_NOT_FOUND")

    candidate = candidate_and_footer[:footer_start].strip("\r\n")
...
def invoke_once(system_prompt, user_prompt):
    ...
    try:
        ...
        if len(stdout) > 0:
            try:
                response = extract_assistant_response(stdout, user_prompt)
                return {
                    "start": start,
                    "end": end,
                    "terminal": True,
                ...
                # Raw stdout was delivered, so this is not a qualifying
                # pre-terminal infrastructure failure and MUST NOT retry.
                # A null response leaves the trial INVALID under the
                # frozen parser's existing handling.
                return {
                    "start": start,
                    "end": end,
                    "terminal": False,
...
def write_attempt_log(
    ...
    append_jsonl(
        log_path,
        {
            ...
            "raw_stdout_b64": base64.b64encode(
                invocation["raw_stdout"]
            ).decode("ascii"),
            "raw_stdout_sha256": sha256_bytes(
                invocation["raw_stdout"]
            ),
            "extraction_error": invocation["extraction_error"],
...
def create_preexec_record(corpus, plan):
    ...
    record = {
        ...
        "v0_3_freeze_manifest_sha256": EXPECTED[FREEZE3],
        "v0_4_freeze_manifest_sha256": EXPECTED[FREEZE4],
...
def verify_preexec(plan):
    if not PREEXEC.is_file():
        raise SystemExit(
            "STOP: RUNNER_PREEXEC_RECORD_v5.json missing. Run --preflight first."
        )
    ...
    if r.get("v0_2_freeze_manifest_sha256") != EXPECTED[FREEZE2]:
        raise SystemExit("STOP: v0.2 freeze identity mismatch in preexec record")

    if r.get("v0_3_freeze_manifest_sha256") != EXPECTED[FREEZE3]:
        raise SystemExit("STOP: v0.3 freeze identity mismatch in preexec record")

    if r.get("v0_4_freeze_manifest_sha256") != EXPECTED[FREEZE4]:
        raise SystemExit("STOP: v0.4 freeze identity mismatch in preexec record")
...
def run_experiment(corpus, target, plan):
    ...
    if RUN_DIR.exists():
        raise SystemExit(
            "STOP: EXECUTION_005 already exists. "
...
def main():
    ...
    if args.preflight:
        if RUN_DIR.exists():
            raise SystemExit(
                "STOP: EXECUTION_005 already exists; observations may exist."
            )
