...
PREEXEC = ROOT / "RUNNER_PREEXEC_RECORD_v4.json"
RUN_DIR = ROOT / "EXECUTION_004"
...
def extract_assistant_response(raw_stdout_bytes, user_prompt):
    ...
    occurrence_count = stdout_text.count(user_prompt)
    if occurrence_count != 1:
        raise ValueError(
            "FROZEN_USER_PROMPT_OCCURRENCE_COUNT_"
            f"{occurrence_count}_EXPECTED_1"
        )

    candidate_and_footer = stdout_text.split(user_prompt, 1)[1]
...
def verify_preexec(plan):
    if not PREEXEC.is_file():
        raise SystemExit(
            "STOP: RUNNER_PREEXEC_RECORD_v4.json missing. Run --preflight first."
        )
...
def run_experiment(corpus, target, plan):
    ...
    if RUN_DIR.exists():
        raise SystemExit(
            "STOP: EXECUTION_004 already exists. "
...
def main():
    ...
    if args.preflight:
        if RUN_DIR.exists():
            raise SystemExit(
                "STOP: EXECUTION_004 already exists; observations may exist."
            )
